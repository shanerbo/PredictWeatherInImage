...
def PrintPredict(Xtrain, ytrain, image):
    ...
    for i, m in enumerate(models):
        ...
        print(modelName[i] + "'s score:" + result[0])


def ReLabel(input):
    if 'Rain' and 'Fog' in input:
        return 'Rain,Fog'
    elif 'Rain' in input:
        return 'Rain'
    elif 'Clear' in input:
        return 'Clear'
    elif 'Cloudy' in input:
        return 'Cloudy'
    elif 'Snow' in input:
        return 'Snow'
    elif 'Fog' in input:
        return 'Fog'
    elif 'Drizzle' in input:
        return 'Drizzle'
    else:
        return input
...
# userInput = 'katkam-20160605160000.jpg'
print(' -------------------------------------------------------------------')
print('|This program take your input image to PREDICT the weather!(Relabel)|')
print(' -------------------------------------------------------------------')
AllGreyImageArray = []
AllColourImageArray = []
AllImageDateArray = []
...
AllCsvDataFrame['NewWeather'] = AllCsvDataFrame['Weather'].apply(ReLabel)
...
X = DateCleaned.iloc[:, 26:].values
y = DateCleaned['NewWeather'].values
...
print('\nUsing GreyScale method to predict Weather:')
GreyImage = classfiy_aHash(LoadGreyImage(userInput))
GreyReshape = np.asarray(GreyImage).reshape(1, -1)

PrintPredict(SeqBug(X_train), y_train, GreyReshape)
print('\nUsing GreyScale method to predict Time:')
PrintPredict(SeqBug(X_train_time), y_train_time, GreyReshape)
...
DateCleanedColour = pd.concat([AllCsvDataFrame.set_index('Date/Time'),
                              ...
                              axis=1, join='inner').reset_index()
XColour = DateCleanedColour.iloc[:, 26:]
yColour = DateCleaned['NewWeather']
X_train_Colour, X_test_Colour, y_train_Colour, y_test_Colour = train_test_split(XColour, yColour)

print('\nUsing Colourful method to predict Weather:')
imageArray = LoadImage(userInput)
ColourReshape = imageArray.reshape(1, -1)
PrintPredict(X_train_Colour, y_train_Colour, ColourReshape)

'''
Grey Image prediction with Time
'''
print('\nUsing Full Colour method to predict Time:')
y_time = DateCleaned['Time'].values
X_train_time, X_test_time, y_train_time, y_test_time = train_test_split(XColour, y_time)

PrintPredict(X_train_time, y_train_time, ColourReshape)
